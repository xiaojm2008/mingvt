...
Page({
  ...
  onLoad: function(options) {
    ...
    this.promtype = options.promtype;
  ...
  loadMore: function(isPull) {
    pageServ.loadMore(promServ.listProm, {
      ...
      shopid: this.userInfo.shopinfo.shopid,
  ...
  addTogger: function(e) {
    //this.showAddDlg(e);
    helperServ.goToPage("../addPromotion/addPromotion");
  },
  modTogger: function(e) {
    var index = e.currentTarget.dataset.index;
   /* this.showAddDlg(e, this.data.inList[parseInt(index)], "修改");*/
    helperServ.goToPage("../addPromotion/addPromotion?prom_id=" + this.data.inList[parseInt(index)]._id);
  },
      ...
      success: (res) => {
        if (res.confirm) {
          promServ.delProm({
            ...
            res.result.success == 1 ? this.onPullDownRefresh() : null;
          }).catch(err => {
            helperServ.showModal({
  ...
  updProm: function(inputlist, prom, cb) {
    ...
    promServ.modProm({
      prom: this.getPromInfo(inputlist)
    }).then(res => {
      helperServ.showModal({
        content: res.result.errMsg
      });
      cb(res.result.success ? null : "e");
