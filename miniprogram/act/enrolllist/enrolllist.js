...
const expServ = require("../../lib/exp/action.js");
...
Page({
  ...
  onLoad: function (options) {
    ...
    this.actioninfo.actionname = options.actionname||'';
    this.actioninfo.actionid = options.actionid || '';
    this.actioninfo.feetype = options.feetype || '';
    this.actioninfo.apprflag = (options.apprflag=='true'?true:false);
    this._qryParams.actionid = options.actionid || '';
  ...
  _goStatis(e) {
    helperServ.goToPage("/statis/act/entry/entry?actionid="+this.actioninfo.actionid);
  },
  callPhone(e) {
  ...
  _expFile(e) {
    ...
    expServ.expEnrollInfo({actionid:this.actioninfo.actionid}).then(res => {     
          ...
          success:(ok)=>{
            if(ok.confirm){
              wx.cloud.downloadFile({
                ...
                helperServ.showToast({title:"下载成功",icon:"none"}) 
              }).catch(error => {
                helperServ.showToast({title:error.errMsg||error.message,icon:"none"})                
              })
            }
          }
        });    
  ...
  onCheck(e){
    var enrollinfo = this.getErollInfo(e);
     /**apprflag,apprstatus : 审批状态100038 0待审核，1pass 2dispass不通过*/
    helperServ.showLoading();
    actionServ.doEnrollCheck({
      enrollid:enrollinfo._id,
      apprstatus:e.currentTarget.dataset.apprstatus}).then(res=>{
        helperServ.hideLoading();
        helperServ.showToast({
