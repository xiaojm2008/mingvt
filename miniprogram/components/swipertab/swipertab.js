  ...
  methods: {
    ...
    scrollByIndex(tabCur, needTransition = true) {
      ...
      let itemWidth = item.width || 0,
        itemLeft = item.left-this.data.padding || 0;
    ...
    sizeChange(newVal, oldVal) {
      if (newVal <= 80) {
		 ...
		 */
    init() {
      this.setData({ windowWidth: app.getWinWidth() || 375 });

      this.createSelectorQuery().selectAll(".tabs-item-child").boundingClientRect((res) => {
        this.setData({
          scrolling: true,
          tabItems: res
        });
