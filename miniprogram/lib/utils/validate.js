const dict = require("./cache.js").dict(true);
var operaterMap = {};
var strToDate = (str) => {
  str = str && str.trim();
  if (!!str) {
    ...
    return new Date(tmp[0], parseInt(tmp[1]) - 1, day[0], hmsArr[0] ? hmsArr[0] : 0, hmsArr[1] ? hmsArr[1] : 0, hmsArr[2] ? hmsArr[2] : 0);
  }
  return null;
}
...
var V = (format, value, fieldid, fromObj, wrapObj, pack) => {
  ...
  if (type != 'object' && type != "array") {
    if (type === 'transfer' && format.runas=='client') {
...
var v1 = function(format, value, fieldid, fromObj, wrapObj, pack) {
  ...
  if ((format.dict || type == 'dict') && required == 'R' && dict) {
    ...
    if (dictlist && !dictlist.some((v, i, a) => {
      ...
      return {
        fieldid: fieldid,
        errMsg: `${name} 的值${value}不在字典[${dictlist.map(v=>v.name).toString()}]中`
      }
    }
  }
}
