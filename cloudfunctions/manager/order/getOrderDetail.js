...
*/
var getOrderDetail = async (event, wxContext) => {
  const {
    order_id,
    actionname
  } = event;
  ...
  /*
  if (!await manageRight.checkUserRight(actionname, userid, shopid)) {
    return {
      errMsg: `您没有[${actionname}]权限`
    }
  }*/
  return await db.collection('xlh_orderdetail').where({shopid:shopid,order_id:order_id}).get();
}
